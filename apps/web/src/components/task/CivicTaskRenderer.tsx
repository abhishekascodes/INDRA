                ...
                {descriptor.declarations.map((decl: CivicTaskDeclaration) => {
                  ...
                  return (
                    <CivicCheckbox
                      ...
                      checked={isAccepted}
                      disabled={isBlocked}
                      onChange={() => !isBlocked && toggleDeclaration(decl.id)}
                      requiredBadge={decl.required}
                      label={decl.text}
                      description={
                        decl.statutoryReference ? (
                          <span className="inline-flex items-center gap-1.5 mt-1 text-xs font-bold text-emerald-800 bg-white/80 px-2.5 py-1 rounded-lg border border-emerald-200/80">
